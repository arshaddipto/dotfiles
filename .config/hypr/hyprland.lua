...
-- Configs that require changes depending on hardware
-- Please review this file and make necessary changes based on your system.
-- Hyprland might fail to launch if not configured correctly.
require("configs.hardware-specific")
...
hl.config({
    general = {
        ...
        col = {
            active_border = colors.primary_fixed_dim,
...
-- Resize windows (in smaller steps) with mainMod + CTRL + SHIFT + arrow keys
hl.bind(
...
hl.bind(mainMod .. " + CTRL + S", hl.dsp.window.move({ workspace = "special:magic", follow = false }))
...
-- Screenshots binds using grim and slurp
local screenshotFullScreen = "grim -"
local screenshotRegion = 'grim -g "$(slurp -w 0)" -'
local screenshotActiveWindow =
    [[hyprctl -j activewindow | jq -r '"\(.at[0]),\(.at[1]) \(.size[0])x\(.size[1])"' | grim -g - -]]
local screenshotFilePath = "~/Pictures/screenshot_$(date +%Y-%m-%d_%H:%M:%S).png"

hl.bind(mainMod .. " + Print", hl.dsp.exec_cmd(screenshotFullScreen .. " | tee " .. screenshotFilePath))
hl.bind(mainMod .. " + CTRL + Print", hl.dsp.exec_cmd(screenshotRegion .. " | tee " .. screenshotFilePath))
hl.bind(mainMod .. " + SHIFT + Print", hl.dsp.exec_cmd(screenshotActiveWindow .. " | tee " .. screenshotFilePath))
hl.bind("Print", hl.dsp.exec_cmd(screenshotFullScreen .. " | wl-copy"))
hl.bind("CTRL + Print", hl.dsp.exec_cmd(screenshotRegion .. " | wl-copy"))
hl.bind("SHIFT + Print", hl.dsp.exec_cmd(screenshotActiveWindow .. " | wl-copy"))
